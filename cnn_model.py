    ...
    with tf.Session() as sess:
        ...
        for epoch in range(training_epochs):
            ...
            for i in range(batch_count):
                ...
                x_batch, y_true_batch, _, cls_batch = data.train.next_batch(1,batch_size)
                x_batch = x_batch.reshape(batch_size, flat_img_size)

                x_test_batch, y_test_batch, _, cls_test_batch = data.test.next_batch(1,batch_size)
...
def predict(img_dir, model_dir, img_size=64, colour_channels=3, batch_size=1):
    ...
    with tf.Session() as sess:
        ...
        x_predict_batch, y_predict_batch, _, cls_predict_batch = data.train.next_batch(2,batch_size=1)

        x_predict_batch = x_predict_batch.reshape(batch_size, flat_img_size)
        tf.summary.image('x_predict', x_predict_batch)
        '''cv2.imshow('input image', x_predict_batch)
        cv2.waitKey(0)'''
        prediction = sess.run([tf.argmax(predict_op, axis=1)], feed_dict={x: x_predict_batch, keep_prob: 1.0})
